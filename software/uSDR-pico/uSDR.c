...
int main() {
	...
	// disable PSU sleep -> less noise
	// Driving high the SMPS mode pin (GPIO23),
	// to force the power supply into PWM mode,
	// can greatly reduce the inherent ripple of the SMPS at light load,
	// and therefore the ripple on the ADC supply.
	gpio_init(PSU_PWM_MODE);
	gpio_set_dir(PSU_PWM_MODE, GPIO_OUT);
	gpio_put(PSU_PWM_MODE, true);
